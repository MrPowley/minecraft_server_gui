...
class ServerPanel:
    def __init__(self):
        ...
        self.server = MinecraftServer.MinecraftServer(
            ...
            self.kill_button,
            self.PWD, self.auto_scroll_var, self.command_entry
        )
    ...
    def load_icons(self):
        ...
        self.play_icon = tk.PhotoImage(data=r"")
        self.stop_icon = tk.PhotoImage(data=r"")
        self.backup_icon = tk.PhotoImage(data=r"")
    ...
    def create_widgets(self):
        ...
        self.player_list_scrollbar = ttk.Scrollbar(bottom_right_frame)
        self.player_list = ttk.Treeview(bottom_right_frame, yscrollcommand=self.player_list_scrollbar.set, show="tree")
        self.player_list_scrollbar.pack(side="right", fill="y")
        self.player_list.pack(fill="both", expand=True)
        self.player_list_scrollbar.configure(command=self.player_list.yview)

        # Associer le menu contextuel au clic droit sur la liste des joueurs
        self.player_list.bind("<Button-3>", self.ct_popup)
        ...
        self.start_button = ttk.Button(top_right_frame, image=self.play_icon)
        self.start_button.grid(row=0, column=0)

        self.stop_button = ttk.Button(top_right_frame, image=self.stop_icon, state=tk.DISABLED)
        self.stop_button.grid(row=0, column=1)

        self.backup_button = ttk.Button(top_right_frame, image=self.backup_icon)
        self.backup_button.grid(row=0, column=2)

        self.kill_button = ttk.Button(top_right_frame, image=self.stop_icon, state=tk.DISABLED)
        self.kill_button.grid(row=0, column=3)
        ...
        # Configuration du serveur (Chemins, RAM...)
        self.create_server_settings(bottom_frame)

    def create_server_settings(self, parent):
        """Crée les entrées pour la configuration du serveur."""
        first_column = ttk.Frame(parent)
        ...
        second_column = ttk.Frame(parent)
        ...
        fourth_column = ttk.Frame(parent)
